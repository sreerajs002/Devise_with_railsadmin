class CategoriesPdf < Prawn::Document
  ...
  def header
    ...
    image "#{Rails.root}/app/assets/images/icon.png", width: 530, height: 150
  end
  ...
  def text_content
   ...
   def categories_rows
    [['#', 'Name','rating','Created_at']] +
      ...
      [category.id, category.cname,category.crating, category.created_at.to_s]
    end
   end

  def table_content    
    table categories_rows do
      row(0).font_style = :bold
      self.header = true
      self.row_colors = ['DDDDDD', 'FFFFFF']
      self.column_widths = [60, 160, 160,160]
    end
  end
